...
global k0 beta deltaz devStepz devStepx Z0 InputFieldG InputFieldH 
global TargetFieldG TargetFieldH Propagator mult loss
...
FieldDisplacement=(12e-06)/2;                   % Target Field Displacement
...
devSizex=38e-6;                                 % device size (transvers)
devSizez=52e-6;                                 % device size (propagation)
...
%%% FIELDS DEFINITION
InputFieldG=hermiteH(0,sqrt(2)*x./w0).*exp(-(x/w0).^2);                    % inizial Gaussian Field
InputFieldH=hermiteH(1,sqrt(2)*x./w0).*exp(-(x/w0).^2);                    % inizial Hermite Gauss Field
...
TargetFieldG=circshift(TargetFieldG,+round(FieldDisplacement/deltax));     % target field displacement for Hermite Gauss field
...
%% Start L2-NSGA
NObjective=2;                                                              % number of objectives, it rapresents the number of modes to sort
...
fun=@BlackBoxfunctionTwoBeams;                                             % function that perform the beam propagation method simulation
iterations=2005;                                                           % number of generation of the genetic algorithm
mutationProb=0.005;                                                        % probability of random mutations
nFOS=100;                                                                 % Number of Linkage structures (FOSs) to consider for the crossover
...
plot(fval(:,1), fval(:,2), 'ro'); title('Fitness values'), xlabel('Gaussian'), ylabel('Hermite Gauss'), grid   %Pareto front plot

[M, ind]=min(abs(fval(:,1)-fval(:,2)));                                    % amongh the final devices we choice the one with the more ballanced performance between gaussian and Hermite Gauss beams
...
figure(2)
subplot(1,2,1), imagesc([abs(FinalFieldG.').^2; abs(FinalFieldH.').^2]), colorbar, axis equal
subplot(1,2,2), imagesc(device'), colorbar, axis equal
F1 = getframe(gcf);

figure(3)
subplot(2,1,1), plot(abs(FinalFieldG(end,:)).^2), hold on
plot(TargetFieldG), hold off, title('Hermite-Gauss 0')
subplot(2,1,2), plot(abs(FinalFieldH(end,:)).^2), hold on
plot(TargetFieldH), hold off, title('Hermite-Gauss 1')
F2 = getframe(gcf);

imwrite(F1.cdata, sprintf('Results/deviceImm1_%s.jpg', ExactTime))         % saves the figures
